...
//@category Analysis
//@author Anthony DeRosa, Syscall 7, LLC
...
/**
 * This script will resolve system calls for ARMv7 (32-bit) binaries.
 * It assumes that the syscall native instruction is used to make system calls,
 * It should be straightforward to modify this script for other cases.
 */
public class ResolveARM32SyscallsScript extends GhidraScript {
	...
	protected void run() throws Exception {
		...
		if (!currentProgram.getLanguage().getProcessor().toString().equals(ARM)) {
			popup("This script is intended for x86 or x64 Linux files, not " + currentProgram.getLanguage().getProcessor().toString());
			return;
		}
		...
		if (size == 64) {
			popup("This script currently only works for 32-bit ARM");
			return;
		}
		else {
			tester = ResolveARM32SyscallsScript::checkARM32Instruction;
			overrideType = RefType.CALLOTHER_OVERRIDE_CALL;
			datatypeArchiveName = "generic_clib";
			callingConvention = "default";

            // uncomment the following line to use the default Linux syscall numbers
		...
		for (Entry<Address, Long> entry : addressesToSyscalls.entrySet()) {
			...
			if (callee == null) {
				String funcName = String.format("syscall_0x%02X", offset);
